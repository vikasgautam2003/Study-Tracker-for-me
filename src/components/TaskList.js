...
// export default function TaskList({ tasks, history, onToggle, onEdit, onDelete }) {
//   const todayKey = getTodayKey();
//   const todayHistory = history[todayKey] || {};
//   const [sortedTasks, setSortedTasks] = useState([]);
...
//       >
//         No tasks added.{" "}
//         <span className="text-blue-400 font-semibold">Click “Add New Task”</span> to begin crushing your goals.
//       </motion.div>
//     );
//   }
...
export default function TaskList({ tasks, history, onToggle, onEdit, onDelete }) {
  ...
  if (sortedTasks.length === 0) {
    return (
      <motion.div
        ...
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.3 }}
        className="solid-card p-8 rounded-2xl text-center text-gray-400 
                   bg-gradient-to-br from-[#0e121b]/90 via-[#0b0f18]/80 to-[#080b12]/90 
                   backdrop-blur-lg border border-gray-700/50 shadow-lg"
      >
        No tasks added.{" "}
        <span className="text-blue-400 font-semibold">Click “Add New Task”</span> to begin crushing your goals.
      </motion.div>
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.4 }}
      className="relative solid-card p-6 rounded-2xl shadow-xl border border-gray-700/40 
                 bg-gradient-to-br from-[#0d121d]/90 via-[#0b101a]/85 to-[#0a0d14]/90 
                 backdrop-blur-xl"
    >
      <div className="absolute inset-x-0 top-0 h-[2px] bg-gradient-to-r 
                      from-blue-500 via-indigo-400 to-blue-600 opacity-60 rounded-t-2xl"></div>

      <div className="flex items-center justify-between mb-5">
        <h2 className="text-2xl font-extrabold text-white tracking-tight neon-text">
          Today's Tasks
        </h2>
        <i className="fas fa-list-check text-blue-400 text-xl"></i>
      </div>
            ...
            return (
              <motion.li
                ...
                className={`group p-4 rounded-xl flex items-center justify-between 
                            ...
                            ${isChecked ? "opacity-70" : "opacity-100"}`}
              >
                <div className="flex items-center w-[70%] sm:w-auto">
                  <motion.input
                    id={task.id}
                    type="checkbox"
                    checked={isChecked}
                    ...
                    className={`h-5 w-5 cursor-pointer rounded-full border-2 transition-all duration-200 
                               ${isChecked ? "bg-green-500 border-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]" : "bg-gray-800 border-gray-600"}`}
                  />
                  <label htmlFor={task.id} className="ml-4 cursor-pointer select-none truncate">
                    <span
                ...
                {/* Buttons — always visible, spaced, and easy to tap */}
                <div className="flex items-center gap-4 shrink-0 ml-3 sm:ml-6">
                  <motion.button
                    onClick={() => onEdit(task)}
                    whileHover={{ scale: 1.1 }}
                    whileTap={{ scale: 0.9 }}
